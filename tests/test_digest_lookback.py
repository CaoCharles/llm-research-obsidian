...
def paper(arxiv_id: str, day: int = 17) -> Paper:
    return Paper(
        arxiv_id=arxiv_id,
        ...
        authors=["Author"],
        categories=["cs.CL"],
        published=datetime(2026, 7, day, 12, tzinfo=timezone.utc),
...
class DigestLookbackTests(unittest.TestCase):
    def test_looks_back_across_empty_weekend_and_deduplicates(self):
        ...
        with patch("cli.fetch_papers_by_date_range", return_value=fetched) as fetch:
    ...
    def test_always_fetches_target_date_at_least_once(self):
        with patch("cli.fetch_papers_by_date_range", return_value=[]) as fetch:
            papers, source_dates = _fetch_digest_candidates(
                categories=["cs.AI"],
                date="2026-07-19",
                lookback_days=0,
